...
@Injectable()
export class BeikeCollector {
  private readonly logger = new Logger(BeikeCollector.name);
  private readonly baseUrl = 'https://sh.ke.com/ershoufang/';

  async collect(
    config: { district?: string; minPrice?: number; maxPrice?: number; maxPages?: number },
    onItem: (property: Partial<Property>) => Promise<void>,
  ): Promise<number> {
    this.logger.log('开始采集贝壳房源数据...');
    ...
    try {
      ...
      this.logger.error(`贝壳采集失败: ${error.message}`);
    }

    this.logger.log(`贝壳采集完成，共 ${collectedCount} 条`);
    return collectedCount;
  }

  private buildUrl(page: number, config: any): string {
    let url = `${this.baseUrl}pg${page}/`;
    
    if (config.district) {
      url = `https://${config.district}.ke.com/ershoufang/pg${page}/`;
    }
  ...
  private parseProperties(html: string): Partial<Property>[] {
    ...
    $('.sellListWrapper li').each((_, element) => {
      try {
        const title = $(element).find('.title a').text().trim();
        const priceText = $(element).find('.priceInfo .totalPrice span').text();
        const price = parseFloat(priceText) || 0;
        const address = $(element).find('.address .houseInfo').text().trim();
        const areaMatch = address.match(/(\d+\.?\d*)平/);
        const area = areaMatch ? parseFloat(areaMatch[1]) : 0;
        const district = $(element).find('.positionInfo a').first().text().trim();
