...
@Injectable()
export class LianjiaCollector {
  private readonly logger = new Logger(LianjiaCollector.name);
  private readonly baseUrl = 'https://sh.lianjia.com/ershoufang/';

  async collect(
    config: { district?: string; minPrice?: number; maxPrice?: number; maxPages?: number },
    onItem: (property: Partial<Property>) => Promise<void>,
  ): Promise<number> {
    this.logger.log('开始采集链家房源数据...');
    ...
    try {
      ...
      this.logger.error(`链家采集失败: ${error.message}`);
    }

    this.logger.log(`链家采集完成，共 ${collectedCount} 条`);
  ...
  private buildUrl(page: number, config: any): string {
    ...
    if (config.district) {
      url = `https://${config.district}.lianjia.com/ershoufang/pg${page}/`;
    }
